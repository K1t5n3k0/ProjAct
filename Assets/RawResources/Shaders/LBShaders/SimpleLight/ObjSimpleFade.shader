...
Shader "ZXShader/Model/ObjSimpleFade"
{
    Properties
    {
        [BeginVertical]
        ...
        _Transparent ("透明度", Range(0, 1)) = 0.5
        ...
        [Indent_add]
        [HideBy(_DeadDosslve, 1)]_DissolveTex ("溶解贴图", 2D) = "white" { }
        [HideBy(_DeadDosslve, 1)][HDR] _DissolveColor("溶解颜色", COLOR) = (1,1,1,1)
        [HideBy(_DeadDosslve, 1)]_Dissolution("动画进度", Range(0, 1)) = 0
        [HideBy(_DeadDosslve, 1)]_DownDistance("死亡下降距离", Float) = 0
        [HideBy(_DeadDosslve, 1)]_DissolveWidth("溶解边缘宽度", Float) = 0
        [Indent_sub]
        [EndVertical]

        [BeginVertical]
        [Toggle(FOG_OFF)] _FogOff("关闭雾效", Float) = 0  
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                //死亡溶解
                #if defined(DEAD_DOSSLVE)
                    o.ModelPos = dot(float3(UNITY_MATRIX_M[1].xyz), v.vertex.xyz);
                    o.vertex.y -= _DownDistance * _Dissolution;
                #endif
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                ApplyLBCaveFog(i.worldPos, 1, col);
            #elif defined(_WILD_LIGHTING_FOG)
                ApplyLBCaveFog(i.worldPos, i.fogCoord.x, col);
            #elif defined(FOG_LINEAR)
